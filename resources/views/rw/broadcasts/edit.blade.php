<x-rw-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-base font-semibold text-gray-900">Edit Pengumuman</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Perbarui isi pesan atau ubah status penayangan.</p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('rw.broadcasts.update', $broadcast) }}" method="POST" class="p-6 md:p-8 space-y-6">
                @csrf
                @method('PUT')
                
                <div>
                    <label class="block text-sm font-bold text-gray-900 mb-2">Judul Pengumuman</label>
                    <input type="text" name="title" value="{{ old('title', $broadcast->title) }}" required class="w-full bg-gray-50/50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block px-4 py-3">
                ...
                <div>
                    ...
                    <textarea name="content" rows="6" required class="w-full bg-gray-50/50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block px-4 py-3">{{ old('content', $broadcast->content) }}</textarea>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex cursor-pointer rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:border-gray-300 has-[:checked]:border-indigo-600 has-[:checked]:ring-1 has-[:checked]:ring-indigo-600">
                            <input type="radio" name="status" value="active" class="sr-only" {{ old('status', $broadcast->status) === 'active' ? 'checked' : '' }}>
                            <div class="flex items-center gap-3">
                        ...
                        <label class="relative flex cursor-pointer rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:border-gray-300 has-[:checked]:border-indigo-600 has-[:checked]:ring-1 has-[:checked]:ring-indigo-600">
                            <input type="radio" name="status" value="inactive" class="sr-only" {{ old('status', $broadcast->status) === 'inactive' ? 'checked' : '' }}>
                            <div class="flex items-center gap-3">
                                <div class="flex h-5 w-5 items-center justify-center rounded-full border-2 border-gray-300 bg-white peer-checked:border-indigo-600 peer-checked:bg-indigo-600">
                                    <span class="h-2.5 w-2.5 rounded-full bg-indigo-600"></span>
                                </div>
                ...
                <div class="pt-6 border-t border-gray-100 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm rounded-xl transition-colors">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
