<x-rw-app-layout title="Buat Tiket Support" header="Buat Tiket Bantuan Baru">
    <div class="max-w-3xl mx-auto py-8 animate-fade-in-up min-h-screen transition-colors duration-300">
        <div class="mb-8">
            <a href="{{ route('rw.tickets.index') }}" class="inline-flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors group">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-2xl shadow-slate-200/40 dark:shadow-none overflow-hidden transition-colors">
            ...
            <form method="POST" action="{{ route('rw.tickets.store') }}" class="p-10 space-y-8" enctype="multipart/form-data">
                @csrf
