<x-rw-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('rw.berita.index') }}" class="p-2 -ml-2 text-slate-500 hover:text-indigo-600 bg-transparent hover:bg-indigo-50 rounded-xl transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Tulis Berita Baru</h2>
                <p class="text-sm text-slate-500 mt-1">Publikasikan informasi atau pengumuman penting untuk warga.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('rw.berita.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            @csrf
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" required 
                        ...
                        placeholder="Contoh: Kerja Bakti Massal Bulan Depan">
                ...
                <div>
                    ...
                    <textarea name="isi" rows="6" required
                        class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('isi') border-red-500 @enderror"
                        placeholder="Tuliskan isi informasi secara lengkap di sini...">{{ old('isi') }}</textarea>
                    @error('isi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <!-- Foto Sampul -->
                <div>
                    <label class="block text-sm font-bold text-slate-900 mb-2">Foto Sampul (Opsional)</label>
                ...
                <div class="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="is_penting" value="1" id="is_penting" {{ old('is_penting') ? 'checked' : '' }} class="w-4 h-4 text-amber-600 bg-white border-amber-300 rounded focus:ring-amber-500">
                    </div>
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">Berita yang Anda publikasikan di sini akan langsung tampil di menu **Berita & Pengumuman** pada Portal Warga.</p>
                    </div>
                </div>
            ...
            <div class="p-8 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
                <a href="{{ route('rw.berita.index') }}" class="px-6 py-3 rounded-xl font-bold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-colors">Batal</a>
