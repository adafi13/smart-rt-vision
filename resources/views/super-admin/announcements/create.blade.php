<x-super-admin-layout title="Buat Pengumuman Baru">
    <div class="max-w-3xl mx-auto py-4">
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden">
            <div class="p-8 border-b border-slate-100 bg-slate-50/30">
                <h3 class="text-sm font-black text-slate-900 uppercase tracking-[0.25em]">Detail Pengumuman</h3>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Pengumuman akan tampil di dashboard seluruh mitra aktif sesuai target</p>
            </div>

            <form method="POST" action="{{ route('super-admin.announcements.store') }}" class="p-8">
                @csrf
                @include('super-admin.announcements._form')
