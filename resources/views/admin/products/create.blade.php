<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Daftarkan Produk/Jasa</h2>
                <p class="text-sm text-slate-500 mt-1">Bantu promosikan UMKM milik warga.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.umkm.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            @csrf
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="nama_produk" value="{{ old('nama_produk') }}" required 
                        ...
                        placeholder="Contoh: Nasi Bakar Bu Tejo">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div x-data="{ 
                        rawVal: '{{ old('harga') }}', 
                        formattedVal: '{{ old('harga') ? number_format(old('harga'), 0, '', '.') : '' }}',
                        formatNumber() {
                ...
                <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="penjual" value="{{ old('penjual') }}" required placeholder="Nama Warga" class="w-full rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" name="whatsapp" value="{{ old('whatsapp') }}" required placeholder="Contoh: 08123456789" class="w-full rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4"
                        class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('deskripsi') border-red-500 @enderror"
                        placeholder="Jelaskan keunggulan produk/jasa ini...">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <!-- Foto Sampul -->
                <div>
                    <label class="block text-sm font-bold text-slate-900 mb-2">Foto Produk (Opsional)</label>
                ...
                <div class="mt-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="is_ready" value="1" id="is_ready" checked class="w-4 h-4 text-emerald-600 bg-white border-emerald-300 rounded focus:ring-emerald-500">
                    </div>
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">UMKM yang Anda tambahkan di sini akan langsung tampil dan dapat dilihat oleh seluruh warga di Portal Warga.</p>
                    </div>
                </div>
