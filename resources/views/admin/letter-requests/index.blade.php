<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
            <div>
                <h2 class="text-sm font-bold text-gray-900">Pengaturan Persuratan</h2>
                <p class="text-xs text-gray-500">Konfigurasi tanda tangan digital Ketua RT</p>
            </div>
            <button x-data="" x-on:click="$dispatch('open-modal', 'pengaturan-ttd')" class="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-xl text-sm font-bold transition-colors">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group align-top">
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex justify-end gap-1.5 transition-opacity">
                                @if($lr->status === 'Disetujui')
                                <button type="button" onclick="triggerPdfPreview('{{ route('admin.pengajuan.pdf', ['letterRequest' => $lr->id, 'preview' => 1]) }}', '{{ route('admin.pengajuan.pdf', $lr->id) }}')" class="p-2 rounded-lg text-emerald-600 hover:bg-emerald-50 transition-colors" title="Cetak Surat PDF">
    ...
    <!-- Modal Pengaturan TTD -->
    <x-modal name="pengaturan-ttd" maxWidth="md">
        <div class="p-6">
            <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-900">Pengaturan Kop & TTD</h2>
                    <p class="text-sm text-gray-500">Tanda tangan digital untuk cetak PDF</p>
                </div>
            </div>

            <form action="{{ route('admin.pengajuan.signature') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="space-y-4">
                    <div>
                        <x-input-label for="rt_name" value="Nama Ketua RT (Lengkap)" />
                        <x-text-input id="rt_name" name="rt_name" type="text" class="mt-1 block w-full" :value="$rtName ?? ''" required placeholder="Contoh: Budi Santoso, S.T." />
                        <p class="text-[10px] text-gray-500 mt-1">Akan tampil di bawah tanda tangan pada surat.</p>
                    </div>

                    <div>
                        <x-input-label value="Tanda Tangan Saat Ini" />
                        <div class="mt-1 p-4 bg-gray-50 rounded-xl border border-gray-200 flex justify-center items-center min-h-[8rem]">
                            @if(isset($rtSignature))
                                <img src="{{ asset('storage/' . $rtSignature) }}" alt="Tanda Tangan" class="max-h-24 max-w-full object-contain">
                            @else
                    ...
                    <div x-data="{
                        ...
                        clearPad() {
                            if(this.pad) {
                                this.pad.clear();
                                this.$refs.signature_data.value = '';
                            }
                        },
                        savePad() {
                            if(this.pad && !this.pad.isEmpty()) {
                                this.$refs.signature_data.value = this.pad.toDataURL('image/png');
                            }
                        }
                    }" @open-modal.window="if ($event.detail === 'pengaturan-ttd') { setTimeout(() => initPad(), 300) }">
                        
                        <x-input-label value="Gambar Tanda Tangan Langsung (Digital)" />
                        <div class="mt-1 relative rounded-xl border-2 border-dashed border-gray-300 bg-white overflow-hidden">
                            <canvas x-ref="canvas" class="w-full h-40 cursor-crosshair touch-none" style="touch-action: none;"></canvas>
                            <div class="absolute bottom-2 right-2">
                                <button type="button" @click="clearPad" class="px-2 py-1 text-[10px] font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-md transition-colors border border-rose-100 shadow-sm">
                                    Hapus / Ulangi
                                </button>
                            </div>
                        </div>
                        <input type="hidden" name="signature_data" x-ref="signature_data" id="signature_data">
                        
                        <div class="my-4 flex items-center gap-3">
                            <div class="h-px bg-gray-200 flex-1"></div>
                            <span class="text-xs text-gray-400 font-bold uppercase tracking-wider">ATAU</span>
                            <div class="h-px bg-gray-200 flex-1"></div>
                        </div>

                        <x-input-label for="rt_signature" value="Upload Tanda Tangan Manual (Opsional)" />
                        <input id="rt_signature" name="rt_signature" type="file" accept="image/png,image/jpeg,image/jpg" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100" />
                        <p class="text-[10px] text-gray-500 mt-1">Gunakan salah satu: menggambar langsung di atas ATAU upload file (maks 2MB).</p>

                        <!-- form submit hook inside the alpine data scope -->
                        <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-100">
                            <button type="button" x-on:click="$dispatch('close')" class="px-5 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-xl shadow-sm hover:bg-gray-50 transition-colors">Batal</button>
                            <button type="submit" @click="savePad" class="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-600 rounded-xl shadow-sm hover:bg-indigo-700 transition-colors">Simpan Pengaturan</button>
                        </div>
                    </div>
                </div>
            </form>
