<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Edit Data Pengurus</h2>
                <p class="text-sm text-slate-500 mt-1">Perbarui informasi jabatan atau status pengurus RT.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.organisasi.update', $staff->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-8 space-y-8">
                ...
                <div x-data="jabatanPicker()">
                    ...
                    <select name="position" required x-model="jabatan" @change="setOrder" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('position') border-red-500 @enderror">
                        ...
                        @foreach($jabatanList as $j)
                            <option value="{{ $j }}" {{ old('position', $staff->position) === $j ? 'selected' : '' }}>{{ $j }}</option>
                        @endforeach
                    </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                        <div>
                            ...
                            <input type="number" name="order_level" min="1" x-ref="orderInput" value="{{ old('order_level', $staff->order_level) }}" required 
                ...
                <!-- Foto Profil -->
                <div>
                    <label class="block text-sm font-bold text-slate-900 mb-2">Ganti Foto Profil (Opsional)</label>
                    @if($staff->photo)
                        <div class="mb-3 relative inline-block rounded-xl overflow-hidden border border-slate-200">
                            <img src="{{ Storage::url($staff->photo) }}" alt="Foto Lama" class="w-32 h-32 object-cover">
                        </div>
                        <p class="text-xs font-semibold text-slate-500 mb-2">Pilih file baru jika ingin mengganti gambar.</p>
                    @endif
                ...
                <div class="mt-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $staff->is_active) ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 bg-white border-emerald-300 rounded focus:ring-emerald-500">
                    </div>
...
function jabatanPicker() {
    ...
    return {
        jabatan: '{{ old("position", $staff->position) }}',
        setOrder() {
