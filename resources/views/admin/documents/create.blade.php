<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Upload Dokumen Baru</h2>
                <p class="text-sm text-slate-500 mt-1">Simpan arsip penting RT ke dalam brankas digital.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.documents.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            @csrf
            
            <div class="p-8 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-lg font-black text-slate-900 mb-1">Informasi Dokumen</h3>
                <p class="text-sm text-slate-500">Silakan isi detail dokumen di bawah ini. Kolom bertanda bintang (<span class="text-red-500 font-bold">*</span>) wajib diisi.</p>
            </div>
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: SK Kelurahan 2026"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" required class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('category') border-red-500 @enderror">
                            ...
                            <option value="notulen" {{ old('category') == 'notulen' ? 'selected' : '' }}>Notulen Rapat</option>
                            <option value="laporan" {{ old('category') == 'laporan' ? 'selected' : '' }}>Laporan (Keuangan/Kegiatan)</option>
                            <option value="surat_masuk" {{ old('category') == 'surat_masuk' ? 'selected' : '' }}>Surat Masuk</option>
                            <option value="surat_keluar" {{ old('category') == 'surat_keluar' ? 'selected' : '' }}>Surat Keluar</option>
                    ...
                    <!-- File Dokumen -->
                    <div>
                        <label class="block text-sm font-bold text-slate-900 mb-2">File Dokumen <span class="text-red-500">*</span></label>
                        ...
                        <p class="text-[11px] text-slate-400 mt-2">Maksimal 10MB. Format: PDF, DOC, XLS, JPG, PNG.</p>
                ...
                <div class="mt-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="is_public" value="1" id="is_public" {{ old('is_public') ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 bg-white border-emerald-300 rounded focus:ring-emerald-500">
                    </div>
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">Dokumen yang bersifat publik akan langsung tampil di Portal Warga dan bisa diunduh kapan saja oleh warga RT Anda.</p>
                    </div>
                </div>
