<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Buat Polling Baru</h2>
                <p class="text-sm text-slate-500 mt-1">Buat jejak pendapat atau musyawarah digital untuk warga.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.polls.store') }}" method="POST" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden" x-data="{ options: ['', ''] }">
            @csrf
            
            <div class="p-8 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-lg font-black text-slate-900 mb-1">Informasi Musyawarah / Polling</h3>
                <p class="text-sm text-slate-500">Silakan isi detail polling di bawah ini. Kolom bertanda bintang (<span class="text-red-500 font-bold">*</span>) wajib diisi.</p>
            </div>
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Pemilihan Ketua RT 05 Periode 2026-2030"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Tuliskan deksripsi pemilihan di sini..."
                        class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('status') border-red-500 @enderror">
                        <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>🟢 Aktif (Bisa divote jika masuk tanggal)</option>
                        <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>🔴 Ditutup (Tidak bisa divote)</option>
                    </select>
                    @error('status') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="border-t border-slate-100 pt-8">
                    <label class="block text-sm font-bold text-slate-900 mb-4">Pilihan Suara (Minimal 2) <span class="text-red-500">*</span></label>
                    <div class="space-y-3">
                        <template x-for="(opt, index) in options" :key="index">
                            <div class="flex gap-3">
                                <input type="text" :name="'options['+index+']'" x-model="options[index]" required
                                    class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm" placeholder="Opsi pilihan...">
                                <button type="button" @click="options.splice(index, 1)" x-show="options.length > 2" 
                                    class="p-3 text-rose-500 bg-rose-50 rounded-xl hover:bg-rose-100 transition-colors shrink-0">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                </button>
                            </div>
                        </template>
                    </div>
                    <button type="button" @click="options.push('')" class="mt-4 inline-flex items-center gap-2 text-sm font-bold text-indigo-600 bg-indigo-50 px-4 py-2.5 rounded-xl hover:bg-indigo-100 transition-colors">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3 mt-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">Polling yang berstatus Aktif dan masuk dalam rentang tanggal akan langsung tampil di Portal Warga dan bisa di-vote.</p>
                    </div>
                </div>
