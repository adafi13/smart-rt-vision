<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.agendas.index') }}" class="p-2 -ml-2 text-slate-500 hover:text-indigo-600 bg-transparent hover:bg-indigo-50 rounded-xl transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Buat Agenda Baru</h2>
                <p class="text-sm text-slate-500 mt-1">Tambahkan kegiatan RT yang akan ditampilkan di Kalender Portal Warga.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.agendas.store') }}" method="POST" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            @csrf
            
            <div class="p-8 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-lg font-black text-slate-900 mb-1">Informasi Kegiatan</h3>
                <p class="text-sm text-slate-500">Isi detail acara. Kolom dengan tanda bintang (<span class="text-red-500 font-bold">*</span>) wajib diisi.</p>
            </div>
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required 
                        ...
                        placeholder="Contoh: Rapat Rutin Bulanan RT 02">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('type') border-red-500 @enderror">
                            ...
                            <option value="kerjabakti" {{ old('type') == 'kerjabakti' ? 'selected' : '' }}>Kerja Bakti</option>
                            <option value="posyandu" {{ old('type') == 'posyandu' ? 'selected' : '' }}>Posyandu</option>
                        </select>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" 
                        class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm"
                        placeholder="Informasi tambahan untuk warga, perlengkapan yang harus dibawa, dll...">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">Agenda yang ditambahkan di sini akan langsung terlihat oleh seluruh warga melalui **Portal Warga** (di bagian Kalender Kegiatan).</p>
                    </div>
                </div>
            ...
            <div class="p-8 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
                <a href="{{ route('admin.agendas.index') }}" class="px-6 py-3 rounded-xl font-bold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-colors">Batal</a>
                <button type="submit" class="px-6 py-3 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">Simpan Agenda</button>
            </div>
        </form>
    </div>
</x-app-layout>
