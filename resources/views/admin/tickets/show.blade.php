<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4 mb-2">
            <a href="{{ route('admin.tickets.index') }}" class="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors bg-white">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-6">
                    ...
                    @foreach($ticket->replies as $reply)
                        @php
                            $isMyReply = $reply->user_id === auth()->id() || $reply->user->tenant_id === res_tenant()->id;
                        @endphp
                        <div class="flex gap-4 {{ $isMyReply ? 'flex-row-reverse text-right' : '' }}">
                            <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white shadow-sm {{ $isMyReply ? 'bg-indigo-600' : 'bg-slate-800' }}">
                                {{ $isMyReply ? strtoupper(substr($reply->user->name, 0, 1)) : 'CS' }}
                            </div>
                            <div class="{{ $isMyReply ? 'bg-indigo-50 border border-indigo-100' : 'bg-gray-50 border border-gray-100' }} p-4 rounded-2xl max-w-[85%] relative">
                                <div class="flex items-center gap-2 mb-1 {{ $isMyReply ? 'justify-end' : '' }}">
                                    <span class="text-xs font-bold {{ $isMyReply ? 'text-indigo-900' : 'text-gray-900' }}">{{ $isMyReply ? 'Anda ('.$reply->user->name.')' : 'Customer Support' }}</span>
                                    <span class="text-[10px] text-gray-400">{{ $reply->created_at->format('H:i') }}</span>
                                </div>
                                <div class="text-sm {{ $isMyReply ? 'text-indigo-800' : 'text-gray-700' }} whitespace-pre-wrap">{{ $reply->message }}</div>
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="pt-6 border-t border-gray-100 text-center">
                        <div class="inline-flex items-center gap-2 text-rose-600 bg-rose-50 px-4 py-2 rounded-lg text-sm font-bold">
                            ...
                            Tiket ini telah ditutup oleh Customer Support.
                        </div>
                    </div>
                    @endif
                </div>
