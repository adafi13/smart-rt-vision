    ...
    <div class="max-w-7xl mx-auto space-y-6" x-data="{ activeTab: 'katalog' }">
        ...
        <div x-show="activeTab === 'katalog'" class="space-y-6 animate-fade-in" style="display: none;">
            ...
            <div class="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <form action="{{ route('admin.inventaris.store') }}" method="POST" class="relative">
                    @csrf
                ...
                @forelse($inventories as $inv)
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-md transition-shadow">
                        ...
                        <div class="border-t border-gray-100 p-3 bg-gray-50/50 flex gap-2">
                            ...
                            <form action="{{ route('admin.inventaris.destroy', $inv) }}" method="POST" class="flex-1" onsubmit="return confirm('Yakin ingin menghapus aset ini secara permanen?')">
                                @csrf @method('DELETE')
                    ...
                    <x-modal name="edit-modal-{{ $inv->id }}" focusable>
                        <div class="p-6">
                            ...
                            <form action="{{ route('admin.inventaris.update', $inv) }}" method="POST" class="space-y-4">
                                @csrf @method('PUT')
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Nama Barang</label>
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                @if($b->status === 'pending')
                                    <div class="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <form action="{{ route('admin.inventaris.borrowings.approve', $b) }}" method="POST">
                                            @csrf
                                        ...
                                        <form action="{{ route('admin.inventaris.borrowings.reject', $b) }}" method="POST">
                                            @csrf
                                ...
                                @elseif($b->status === 'approved')
                                    <form action="{{ route('admin.inventaris.borrowings.return', $b) }}" method="POST" class="opacity-0 group-hover:opacity-100 transition-opacity">
                                        @csrf
            ...
            <div class="md:hidden space-y-3">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative">
                    ...
                    <div class="p-3 bg-gray-50/50 border-t border-gray-50 flex gap-2 pl-5">
                        <form action="{{ route('admin.inventaris.borrowings.approve', $b) }}" method="POST" class="flex-1">
                            @csrf
                        ...
                        <form action="{{ route('admin.inventaris.borrowings.reject', $b) }}" method="POST" class="flex-1">
                            @csrf
                    ...
                    @elseif($b->status === 'approved')
                    <div class="p-3 bg-gray-50/50 border-t border-gray-50 flex gap-2 pl-5">
                        <form action="{{ route('admin.inventaris.borrowings.return', $b) }}" method="POST" class="w-full">
                            @csrf
