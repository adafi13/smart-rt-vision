<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-900 leading-tight">Ubah Data Iuran</h2>
                <p class="text-sm text-slate-500 mt-1">Perbarui rincian transaksi kas.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('admin.iuran.update', $contribution) }}" method="POST" class="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                ...
                <div class="bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider">
                    Edit Mode
                </div>
            </div>
            ...
            <div class="p-8 space-y-8">
                <div>
                    ...
                    <select name="family_id" required class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('family_id') border-red-500 @enderror">
                        @foreach($families as $f)
                            <option value="{{ $f->id }}" {{ old('family_id', $contribution->family_id) == $f->id ? 'selected' : '' }}>{{ $f->nomor_kk }} — {{ $f->nama_kepala_keluarga }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="jenis_iuran" value="{{ old('jenis_iuran', $contribution->jenis_iuran) }}" required
                    ...
                    <div x-data="{ 
                            raw: '{{ old('jumlah', $contribution->jumlah) }}',
                            formatted: '',
                            init() {
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="periode" value="{{ old('periode', $contribution->periode->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_bayar" value="{{ old('tanggal_bayar', $contribution->tanggal_bayar->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                        class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors px-4 py-3 text-sm @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $contribution->keterangan) }}</textarea>
